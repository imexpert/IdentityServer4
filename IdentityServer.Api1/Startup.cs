        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
                .AddJwtBearer(JwtBearerDefaults.AuthenticationScheme, s =>
                {
                    ...
                    s.Audience = "resource_api1";
                    s.RequireHttpsMetadata = false;
                });

            services.AddAuthorization(s =>
            {
                s.AddPolicy("ReadProduct", pol =>
                {
                    pol.RequireClaim("scope", "api1.read");
                });

                s.AddPolicy("UpdateOrCreate", pol =>
                {
                    pol.RequireClaim("scope", new[] { "api1.update", "api1.create" });
                });
            });
